...
def detect_emotion():
    ...
    for (x, y, w, h) in faces:
        ...
        probabilities = {emotion_labels[i]: float(f"{prob:.2f}") for i, prob in enumerate(prediction)}
        break

    return render_template("index.html", emotion=emotion_result, feedback=feedback, probabilities=probabilities)
